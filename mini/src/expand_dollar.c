...
/*   Updated: 2022/06/30 16:39:51 by ael-kouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_from_env(char *from_env, char **env)
{
	char	**spl_doll;
	int		i;
	char	*str;
	...
	i = 0;
	spl_doll = ft_split(from_env, '$');
	while(spl_doll[i])
	{
		// tmp = spl_doll[i];
		str = get_char_to_find(spl_doll[i], env);
		// free(tmp);
		i++;
	}
	// i = 0;
	...
	// while(spl_doll[i + 1])
	// {
	// 	str = ft_strjoin(spl_doll[i], spl_doll[i + 1]);
	// 	i++;
	// }
	// return(str);
	return(str);
...
char	*exp_d(char **spl, int i, char **env)
{
	char	*tmp;
	char	*finalresult;
	while(spl[i])
	{
		tmp = spl[i];
		spl[i] = get_char_to_find(spl[i], env);
		printf("%s\n", spl[i]);
		free(tmp);
		i++;
	}
	i = 0;
	finalresult = ft_strdup("\0");
	while(spl[i + 1])
	{
		tmp = finalresult;
		finalresult = ft_strjoin(spl[i], spl[i + 1]);
		free(tmp);
	}
	return(finalresult);
}
